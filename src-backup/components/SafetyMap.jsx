...
export default function SafetyMap({
  ...
  useEffect(() => {
    ...
    if (mapMode === 'mysuru') {
      ...
      dataToRender.forEach((a) => {
        ...
        const color = severity >= 5 ? '#ef4444' : severity >= 3 ? '#f97316' : '#eab308';
        ...
        circle.bindPopup(`
          <div style="color: #1a1a2e; font-family: Inter, sans-serif; padding: 2px;">
            <div style="font-weight: 700; font-size: 13px; margin-bottom: 2.5px;">🔥 Historical Accident</div>
      ...
      karnatakaCells.forEach((c) => {
        ...
        const color = risk >= 80 ? '#ef4444' : risk >= 30 ? '#f97316' : '#eab308';
        ...
        circle.bindPopup(`
          <div style="color: #1a1a2e; font-family: Inter, sans-serif; padding: 2px;">
            <div style="font-weight: 700; font-size: 13px; margin-bottom: 2px;">🏛️ Karnataka Risk Cell</div>
            <div style="font-size: 11px; font-weight: 700; color: #ef4444;">Risk Score: ${risk}</div>
  ...
  useEffect(() => {
    ...
    seedBlindSpots.forEach((bs) => {
      ...
      dot.bindPopup(`
        <div style="color: #1a1a2e; font-family: Inter, sans-serif; padding: 2px;">
          <div style="font-weight: 700; font-size: 13px; margin-bottom: 2px;">👁️ Blind Spot</div>
  ...
  useEffect(() => {
    ...
    clusters.forEach((cluster) => {
      ...
      const markerColor = confidence.color || '#eab308';
      const typeEmoji = mainFlag.type === 'hazard' ? '⚠️' : mainFlag.type === 'blindspot' ? '👁️' : '⚡';

      const marker = L.circleMarker([cluster.center.lat, cluster.center.lng], {
        radius: 6 + Math.min(6, cluster.flags.length * 1.5),
      ...
      marker.bindPopup(`
        <div style="color: #1a1a2e; font-family: Inter, sans-serif; padding: 2px;">
          <div style="font-weight: 700; font-size: 13px; margin-bottom: 2px;">
            ${typeEmoji} ${mainFlag.subtype}
          </div>
          <div style="font-size: 12px; color: #666; margin-bottom: 2px;">
            ${confidence.label} · ${confidence.independentCount || cluster.flags.length} report(s)
          </div>
          <div style="font-size: 10px; color: #999; font-style: italic;">
            Reported: ${new Date(mainFlag.timestamp).toLocaleTimeString()}
          </div>
        </div>
      `);
